...
void main() {
  group('WhisperOutputParser', () {
    ...
    test('parseRawText parses standard MM:SS.mmm and HH:MM:SS.mmm timestamps', () {
      const raw = '''
...
''';
    ...
    test('parseRawText falls back to single segment when text has no timestamps', () {
    ...
    test('mergeSegments deduplicates identical words at chunk overlap boundary', () {
      final segments = [
        SubtitleSegment(
          index: 0,
    ...
    test('mergeSegments clamps overlapping timestamps monotonically', () {
      ...
      expect(merged.length, 2);
      expect(merged[0].endTime, const Duration(seconds: 4)); // clamped to current.startTime
